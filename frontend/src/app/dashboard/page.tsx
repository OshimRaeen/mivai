...
import { 
  ...
  Calendar, CheckCircle2, XCircle, Activity, BrainCircuit 
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  // 🚀 NEW: State for interview history
  ...
  // 🚀 NEW: Fetch interview history from the Express backend
  useEffect(() => {
  ...
  return (
    <main className="min-h-screen bg-[#f5f5f7] text-[#1d1d1f] font-sans selection:bg-blue-200 pb-20 relative overflow-hidden">
      ...
      <motion.div variants={stagger} initial="hidden" animate="show" className="max-w-5xl mx-auto px-6 py-20 space-y-16 relative z-10">
        ...
        {/* 🚀 UPGRADED: Liquid Glass Dynamic Carousel */}
        <motion.div variants={fadeUp} className="space-y-6 relative">
          ...
          {loadingSessions ? (
            ...
            <div className="bg-white/40 backdrop-blur-3xl border border-white/60 rounded-[2rem] p-16 shadow-[0_8px_30px_rgb(0,0,0,0.04)] text-center flex flex-col items-center gap-4">
              ...
              <p className="text-[#1d1d1f]/60 font-medium text-lg">Your completed interviews will appear here.</p>
            </div>
          ) : (
            <div className="flex overflow-x-auto snap-x snap-mandatory gap-6 pb-12 pt-4 px-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {interviews.map((interview) => {
                const isHire = interview.evaluation.finalVerdict.toLowerCase().includes('hire') && 
                              !interview.evaluation.finalVerdict.toLowerCase().includes('no hire');
                
                return (
                  <div 
                    key={interview._id}
                    onClick={() => router.push(`/dashboard/results/${interview._id}`)}
                    className="snap-center md:snap-start shrink-0 w-[85vw] md:w-[380px] cursor-pointer group bg-white/40 backdrop-blur-3xl border border-white/60 rounded-[2rem] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-500 flex flex-col"
                  >
                    {/* Top row: Status & Date */}
                    <div className="flex justify-between items-start mb-8">
                      <div className={`px-4 py-1.5 rounded-full border text-xs font-semibold flex items-center gap-2 ${
                        isHire ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-red-50 border-red-200 text-red-600'
                      }`}>
                    ...
                    <div className="space-y-2 mb-8">
                      <h3 className="font-semibold text-2xl tracking-tight text-[#1d1d1f] group-hover:text-blue-600 transition-colors line-clamp-1">
                        {interview.category}
                      </h3>
                      <p className="text-[#1d1d1f]/50 text-sm font-medium tracking-tight">
                        {interview.difficulty} • {interview.jobRole}
                      </p>
                    </div>
                    ...
                    <div className="flex justify-between items-end mt-auto pt-6 border-t border-[#1d1d1f]/5">
                      <div>
                        <p className="text-xs text-[#1d1d1f]/40 font-medium mb-1">Overall Rating</p>
                        <p className="text-3xl font-semibold tracking-tight text-[#1d1d1f]">
                          {interview.evaluation.overallScore}
                        </p>
                      </div>
                      <div className="w-10 h-10 rounded-full bg-white/60 border border-white/80 shadow-sm flex items-center justify-center text-[#1d1d1f]/50 group-hover:bg-blue-500 group-hover:border-blue-500 group-hover:text-white transition-all duration-300">
