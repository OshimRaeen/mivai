...
export default function CodeEditor() {
  ...
  useEffect(() => {
    ...
    if (interviewConfig?.category) {
      // 1. Check if there is existing data to recover
      const savedCode = sessionStorage.getItem('recovery_code');
      const savedLang = sessionStorage.getItem('recovery_lang');
      const savedOutput = sessionStorage.getItem('recovery_output');

      if (savedCode && savedLang) {
        // 🚀 RECOVERY MODE: Inject the exact state before the reload
        setEditorLanguage(savedLang);
        setEditorCode(savedCode);
        if (savedOutput) setOutput(savedOutput);
      } else {
        // 🚀 FRESH START MODE: Apply standard boilerplate based on category
  ...
  // ─── 🚀 THE FIX: AUTO-SAVE LANGUAGE & OUTPUT ───────────────────────────────
  useEffect(() => {
    if (isMounted && editorLanguage) {
      sessionStorage.setItem('recovery_lang', editorLanguage);
    }
  }, [editorLanguage, isMounted]);

  useEffect(() => {
    if (isMounted && output) {
      sessionStorage.setItem('recovery_output', output);
    }
  }, [output, isMounted]);
